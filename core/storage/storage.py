...
class GCSBackend(StorageBackend):
    ...
    def __init__(self, bucket_name: Optional[str] = None) -> None:
        ...
        # _storage is created lazily on first use so __init__ stays sync-safe.
        self._storage = None
    ...
    async def exists(self, blob_name: str) -> bool:
        ...
        try:
            ...
            # gcloud-aio raises on 404 — any exception means does not exist
            return False
...
# ─────────────────────────────────────────────────────────────────────────────
# Factory — module-level singleton, one HTTP session per process
# ─────────────────────────────────────────────────────────────────────────────
